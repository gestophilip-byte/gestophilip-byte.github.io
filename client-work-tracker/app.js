...
function earned(l){if(l.billAmount!==''&&l.billAmount!==null&&l.billAmount!==undefined&&!Number.isNaN(Number(l.billAmount)))return Number(l.billAmount);const c=client(l.clientId);return hours(l)*(c?Number(c.rate)||0:0)}
...
function persist(){localStorage.setItem(KEYS.clients,JSON.stringify(clients));localStorage.setItem(KEYS.logs,JSON.stringify(logs));localStorage.setItem(KEYS.payments,JSON.stringify(payments));localStorage.setItem(KEYS.tasks,JSON.stringify(tasks));localStorage.setItem(KEYS.invoices,JSON.stringify(invoices));render()}
...
function loadData(){
 ...
 if(!clients.length){
   ...
   if(oc.length){
     clients=oc.map(c=>({id:c.id||uid(),company:c.company||'',contact:c.contact||'',billingType:'hourly',rate:Number(c.rate)||0,fixedRate:0,method:c.method||'',schedule:c.schedule||'',nextPay:c.nextPay||'',weeklyTarget:Number(c.expectedHours||0)*5||0,status:c.status||'active',notes:c.notes||''}));
     ...
     clients=[
       {id:uid(),company:'Merch Cartel / Apparelology',contact:'Tony',billingType:'hourly',rate:5,fixedRate:0,method:'',schedule:'',nextPay:'',weeklyTarget:20,status:'active',notes:''},
       {id:uid(),company:'Digital Sea Creative',contact:'Brandon',billingType:'hourly',rate:0,fixedRate:0,method:'',schedule:'',nextPay:'',weeklyTarget:0,status:'active',notes:''}
     ];
   }
   persist();
 }
...
function renderClients(){
 ...
 $('clientRows').innerHTML=arr.length?arr.map(c=>{
   ...
   const rate=c.billingType==='fixed'?(money(c.fixedRate)+' project'):money(c.rate)+'/hr';
...
function renderLogs(){
 ...
 $('logRows').innerHTML=arr.length?arr.map(l=>{const c=client(l.clientId);return '<tr><td>'+fmtDate(l.date)+'</td><td><div class="company">'+esc(c?.company||'Deleted client')+'</div></td><td>'+fmtTime(l.timeIn)+' → '+fmtTime(l.timeOut)+(l.timeIn&&!l.timeOut?'<div class="smallmuted"><span class="pill open">Open</span></div>':'')+'</td><td>'+Number(l.breakMin||0)+' min</td><td>'+hours(l).toFixed(2)+'</td><td>'+esc(l.task||'—')+'</td><td class="money">'+money(earned(l))+'</td><td><div class="row-actions"><button class="btn small" onclick="editWork(\''+l.id+'\')">Edit</button><button class="btn small red" onclick="deleteWork(\''+l.id+'\')">Delete</button></div></td></tr>'}).join(''):'<tr><td colspan="8" class="empty">No work logs for this period.</td></tr>';
}
...
function resetClient(){['cId','cCompany','cContact','cMethod','cSchedule','cNextPay','cWeeklyTarget','cNotes','cRate','cFixedRate'].forEach(id=>$(id).value='');$('cBilling').value='hourly';$('cStatus').value='active';toggleBilling()}
...
function editClient(id){const c=client(id);if(!c)return;resetClient();$('clientTitle').textContent='Edit Client';$('cId').value=c.id;$('cCompany').value=c.company;$('cContact').value=c.contact||'';$('cBilling').value=c.billingType||'hourly';$('cRate').value=c.rate||0;$('cFixedRate').value=c.fixedRate||0;$('cMethod').value=c.method||'';$('cSchedule').value=c.schedule||'';$('cNextPay').value=c.nextPay||'';$('cWeeklyTarget').value=c.weeklyTarget||'';$('cStatus').value=c.status||'active';$('cNotes').value=c.notes||'';toggleBilling();$('clientDialog').showModal()}
...
function resetWork(){['wId','wIn','wOut','wManual','wBill','wTask'].forEach(id=>$(id).value='');$('wDate').value=localDate();$('wBreak').value=0;syncWorkCalc()}
...
function editWork(id){const l=logs.find(x=>x.id===id);if(!l)return;resetWork();populateSelects();$('workTitle').textContent='Edit Work Log';$('wId').value=l.id;$('wClient').value=l.clientId;$('wDate').value=l.date;$('wIn').value=l.timeIn||'';$('wOut').value=l.timeOut||'';$('wBreak').value=l.breakMin||0;$('wManual').value=l.manualHours??'';$('wBill').value=l.billAmount??'';$('wTask').value=l.task||'';syncWorkCalc();$('workDialog').showModal()}
...
function syncWorkCalc(){const h=$('wManual').value!==''?Number($('wManual').value)||0:calcTime($('wIn').value,$('wOut').value,$('wBreak').value);const c=client($('wClient').value);const amt=$('wBill').value!==''?Number($('wBill').value)||0:h*(c?Number(c.rate)||0:0);$('wCalculated').textContent=h.toFixed(2)+' hours • '+money(amt)}
...
function exportCsv(){const h=['Date','Company','Contact','Time In','Time Out','Break Minutes','Hours','Task','Billable USD'];const rows=logs.map(l=>{const c=client(l.clientId);return[l.date,c?.company||'',c?.contact||'',l.timeIn,l.timeOut,l.breakMin,hours(l),l.task,earned(l)]});download('philip-work-logs.csv',[h,...rows].map(r=>r.map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(',')).join('\n'),'text/csv')}
...
$('clientForm').onsubmit=e=>{e.preventDefault();const id=$('cId').value||uid(),obj={id,company:$('cCompany').value.trim(),contact:$('cContact').value.trim(),billingType:$('cBilling').value,rate:Number($('cRate').value)||0,fixedRate:Number($('cFixedRate').value)||0,method:$('cMethod').value.trim(),schedule:$('cSchedule').value.trim(),nextPay:$('cNextPay').value,weeklyTarget:Number($('cWeeklyTarget').value)||0,status:$('cStatus').value,notes:$('cNotes').value.trim()};const i=clients.findIndex(c=>c.id===id);if(i>=0)clients[i]=obj;else clients.push(obj);persist();$('clientDialog').close()};
['wIn','wOut','wBreak','wManual','wBill'].forEach(id=>$(id).addEventListener('input',syncWorkCalc));$('wClient').addEventListener('change',prefillBill);
$('workForm').onsubmit=e=>{e.preventDefault();const id=$('wId').value||uid(),obj={id,clientId:$('wClient').value,date:$('wDate').value,timeIn:$('wIn').value,timeOut:$('wOut').value,breakMin:Number($('wBreak').value)||0,manualHours:$('wManual').value===''?'':Number($('wManual').value),billAmount:$('wBill').value===''?'':Number($('wBill').value),task:$('wTask').value.trim(),createdAt:Date.now()};const i=logs.findIndex(l=>l.id===id);if(i>=0)logs[i]=obj;else logs.push(obj);persist();$('workDialog').close()};
